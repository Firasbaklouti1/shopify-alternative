...
export default function ProductGrid({
  ...
  const providedProducts = puck?.metadata?.products as Product[] | undefined;
  ...
  useEffect(() => {
    ...
    async function loadProducts() {
      try {
        ...
        if (collection_handle) params.set('category', collection_handle);

        const res = await fetch(`${apiUrl}/api/v1/storefront/${storeSlug}/products?${params}`);
        if (res.ok) {
    ...
    loadProducts();
  }, [storeSlug, apiUrl, collection_handle, limit, providedProducts]);
    ...
    if (puck?.isEditing) {
      return (
        <section className="py-12 px-6 md:px-12 text-center text-gray-400">
          <p>Product Grid (no products available)</p>
        </section>
      );
    }
    return null;
